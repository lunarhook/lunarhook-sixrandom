...
@implementation plumberFMResultSet
@synthesize query=_query;
@synthesize statement=_statement;

+ (instancetype)resultSetWithStatement:(plumberFMStatement *)statement usingParentDatabase:(plumberFMDatabase*)aDB {
    
    plumberFMResultSet *rs = [[plumberFMResultSet alloc] init];
...
- (BOOL)nextWithError:(NSError **)outErr {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        NSLog(@"%s:%d plumberDatabase busy (%@)", __FUNCTION__, __LINE__, [_parentDB databasePath]);
        NSLog(@"plumberDatabase busy");
        if (outErr) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"plumberFMDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
...
- (void)setParentDB:(plumberFMDatabase *)newDb {
    _parentDB = newDb;
}
