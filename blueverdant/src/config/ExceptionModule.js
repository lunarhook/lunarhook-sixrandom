...
const { NativePlumber } = NativeModules;
...
export const setJSExceptionHandler = (customHandler = noop, allowedInDevMode = false) => {
  ...
  if (allowed) {
    ...
    console.error = (message, error) => {
      global.ErrorUtils.reportError(error); // sending console.error so that it can be caught
      
      console.log("setJSExceptionHandler",message)
    }
  } else {
