...
class taiyiHistoryPage extends React.Component {
  ...
  */
  ...
  _starRow(rowData){
    HistoryArrayGroup.loadid('taiyi', rowData.id ).then(async(ret) => {
      var Jobj = JSON.parse(ret);
      Jobj.star = true == Jobj.star ? false : true
      Jobj.sync = false
      ret = JSON.stringify(Jobj)
      let T = await UserModule.SyncFileServer("taiyi",rowData.id,ret)
        if(undefined !=T && 2000==T.code)
        {
            ret = HistoryArrayGroup.MakeJsonSync(ret)
        }
      await HistoryArrayGroup.saveid('taiyi', rowData.id,ret)
      this.refreshlist()
    })
  }
  _updateRow(rowData) {
  ...
  async _deletehistory(rowData) {
    HistoryArrayGroup.loadid('taiyi', rowData.id).then(async (ret) => {
      var Jobj = JSON.parse(ret);
      let T = await UserModule.SyncFileServer("taiyi", rowData.id, "")
      if (undefined != T && 2000 == T.code) {
        T.data.forEach(async (element) => {
          filename = element.File
          if (-1 != filename.indexOf(String(rowData.id)) && true == element.Del) {
            await HistoryArrayGroup.remove('taiyi', rowData.id);
          }
        });
      }
      else {
