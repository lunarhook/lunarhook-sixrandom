...
luckyinfo["天医"] = "天医吉代表钱财，婚姻，业绩，财富"
luckyinfo["生气"] = "生气吉代表贵人，亲朋，同事，人脉"
luckyinfo["延年"] = "延年吉代表事业，专业，能力，健康"
luckyinfo["伏位"] = "伏位吉代表延续，积蓄，被动，耐力"
luckyinfo["绝命"] = "绝命凶代表投资，开支，破财，情绪"
luckyinfo["祸害"] = "祸害凶代表口舌，小人，伤灾，疾病"
luckyinfo["五鬼"] = "五鬼凶代表变动，矫捷，血光，灵动"
luckyinfo["六煞"] = "六煞凶代表桃花，忧郁，时尚，情商"
...
class NumberMainPage extends React.Component {
  ...
  clear(val) {
    ...
    this.setState({ number: "", info: "", extra: "", spextra: "", color: false, switchstate: "男" == val ? true : false, selectedValue: val })
  }
  color(val) {
  ...
  renderItem(item,pthis) {
    var c = IconConfig.colorblack
    if (-1 != item.item.indexOf("天医")) {
      c = IconConfig.colorfire
    }
    if (-1 != item.item.indexOf("延年")) {
      c = IconConfig.colororange
    }
    else if (-1 != item.item.indexOf("生气")) {
      c = IconConfig.colorCoral
    }
    else if (-1 != item.item.indexOf("伏位")) {
      c = IconConfig.colorgold
    }
    else if (-1 != item.item.indexOf("六煞")) {
      c = IconConfig.colorgreen
    }
    else if (-1 != item.item.indexOf("祸害")) {
      c = IconConfig.colorclaygreen
    }
    else if (-1 != item.item.indexOf("五鬼")) {
      c = IconConfig.colorstartblue
    }
    else if (-1 != item.item.indexOf("绝命")) {
      c = IconConfig.colorblue
    }

    if (pthis== false) {
      c = IconConfig.colorblack
    }
    return (
        ...
        <ScrollView ref="location" style={{ backgroundColor: '#ffffff' }}>
          <View style={styles.container} >
            ...
            <FlatList
              ...
              renderItem={(item)=>{return this.renderItem(item,this.state.color)}}
            //numColumns ={8}
            //horizontal={true}
