...
class MarryHistoryPage extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: false,
  ...
  */
  ...
  _starRow(rowData) {
    HistoryArrayGroup.loadid('Marry', rowData.id).then(async (ret) => {
      var Jobj = JSON.parse(ret);
      Jobj.star = true == Jobj.star ? false : true
      Jobj.sync = false
      ret = JSON.stringify(Jobj)
      let T = await UserModule.SyncFileServer("Marry", rowData.id, ret)
      if (undefined != T && 2000 == T.code) {
        ret = HistoryArrayGroup.MakeJsonSync(ret)
      }
      await HistoryArrayGroup.saveid('Marry', rowData.id, ret)
      this.refreshlist();
    })
  }
  _updateRow(rowData) {
  ...
  async _deletehistory(rowData) {
    HistoryArrayGroup.loadid('Marry', rowData.id).then(async (ret) => {
      var Jobj = JSON.parse(ret);
      let T = await UserModule.SyncFileServer("Marry", Jobj.id, "")
      if (undefined != T && 2000 == T.code) {
        T.data.forEach(async (element) => {
          filename = element.File
          if (-1 != filename.indexOf(String(rowData.id))) {
            await HistoryArrayGroup.remove("Marry", rowData.id);
          }
        });
      }
      else {
                    ...
                    >
                      <Card style={{ width: width - 20, paddingLeft: 10 }} >
                        <TouchableOpacity onPress={() => { this.onPress(data.item) }}>
                          ...
                          <Card.Body>
                            <View >
                            <Text style={{ fontSize: FontStyleConfig.getFontApplySize() + 14, marginLeft: 16, marginRight: 16 }}>{data.item.EightDatemale}</Text>
                              <Text style={{ fontSize: FontStyleConfig.getFontApplySize() + 14, marginLeft: 16, marginRight: 16 }}>{data.item.EightDatefemale}</Text>
                              <Text style={{ fontSize: FontStyleConfig.getFontApplySize() + 14, marginLeft: 16, marginRight: 16 }}>{data.item.birthmale}</Text>
                              <Text style={{ fontSize: FontStyleConfig.getFontApplySize() + 14, marginLeft: 16, marginRight: 16 }}>{data.item.birthfemale}</Text>
                            </View>
                          </Card.Body>
                        </TouchableOpacity>
                      </Card>
...
module.exports = MarryHistoryPage;  