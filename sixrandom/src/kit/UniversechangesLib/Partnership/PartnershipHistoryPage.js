...
class PartnershipHistoryPage extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: false,
  ...
  */
  ...
  _starRow(rowData) {
    HistoryArrayGroup.loadid('Partnership', rowData.id).then(async (ret) => {
      var Jobj = JSON.parse(ret);
      Jobj.star = true == Jobj.star ? false : true
      Jobj.sync = false
      ret = JSON.stringify(Jobj)
      let T = await UserModule.SyncFileServer("Partnership", rowData.id, ret)
      if (undefined != T && 2000 == T.code) {
        ret = HistoryArrayGroup.MakeJsonSync(ret)
      }
      await HistoryArrayGroup.saveid('Partnership', rowData.id, ret)
      this.refreshlist();
    })
  }
  _updateRow(rowData) {
  ...
  async _deletehistory(rowData) {
    HistoryArrayGroup.loadid('Partnership', rowData.id).then(async (ret) => {
      var Jobj = JSON.parse(ret);
      let T = await UserModule.SyncFileServer("Partnership", Jobj.id, "")
      if (undefined != T && 2000 == T.code) {
        T.data.forEach(async (element) => {
          filename = element.File
          if (-1 != filename.indexOf(String(rowData.id))) {
            await HistoryArrayGroup.remove("Partnership", rowData.id);
          }
        });
      }
      else {
                    ...
                    >
                      <Card style={{ width: width - 20, paddingLeft: 10 }} >
                        <TouchableOpacity onPress={() => { this.onPress(data.item) }}>
                          <Card.Header
                            title={<Text style={{ fontSize: FontStyleConfig.getFontApplySize() + 14 }}>{data.item.id}</Text>}
                            //thumbStyle={{ width: 30, height: 30 }}
                            thumb={true == data.item.star ? IconConfig.IconStar : IconConfig.IconUStar}
                            extra={<Text style={{ fontSize: FontStyleConfig.getFontApplySize() + 14 }}>{data.item.tip}</Text>} />
                          <Card.Body>
                            <View >
                            <Text style={{ fontSize: FontStyleConfig.getFontApplySize() + 14, marginLeft: 16, marginRight: 16 }}>{data.item.EightDateleader}</Text>
                              <Text style={{ fontSize: FontStyleConfig.getFontApplySize() + 14, marginLeft: 16, marginRight: 16 }}>{data.item.EightDatePartnership}</Text>
                              <Text style={{ fontSize: FontStyleConfig.getFontApplySize() + 14, marginLeft: 16, marginRight: 16 }}>{data.item.birthleader}</Text>
                              <Text style={{ fontSize: FontStyleConfig.getFontApplySize() + 14, marginLeft: 16, marginRight: 16 }}>{data.item.birthPartnership}</Text>
                            </View>
                          </Card.Body>
                        </TouchableOpacity>
                      </Card>
...
module.exports = PartnershipHistoryPage;  