...
const fixedItems = [0];
let LunarCourseConfigPageController = null
class LunarCourseConfigPage extends React.Component {
    constructor(props) {
        super()
    ...
    static navigationOptions = ({ navigation }) => {
        ...
        return {
            title: RouteConfig["LunarCourseConfigPage"].name,
        }
    }
    refreshlist() {
            ...
            <View style={StyleConfig.container}>
                <ScrollView
                    ...
                    <View style={styles.hurdle}>
                        <Text style={StyleConfig.hurdle_title}>{'课程模式'}</Text>
                    ...
                    <View style={[styles.hurdle, { justifyContent: 'flex-start', marginTop: 40 }]}>
                        <Text style={StyleConfig.hurdle_title}>{'课程列表'}</Text>
                    </View>
                    <DragSortableView
    ...
    onSelectedModeClickItem = async (data, item, index) => {
        // delete, data 是最新的数据
        var selectmode = CourseConfig.getCourseSelectMode()
        var itemsrandom = CourseConfig.getCourseRandom()
        console.log("CourseConfigselectmode", selectmode)
        var curselectmode = JSON.parse(JSON.stringify(selectmode))
        curselectmode[index].isSelect = false
        await HistoryArrayGroup.save("CourseConfigselectmode", curselectmode[index].title)
    ...
    async SaveCourseConfigs() {
        ...
        HistoryArrayGroup.removeall("CourseConfig").then(R => {
            HistoryArrayGroup.saveid("CourseConfig", 0, items).then(T1 => {
                navigate(RouteConfig['LunarCoursePage'].route, { text: "refresh" })
                Toast.success("保存成功")
            })
        })
    }
...
module.exports = LunarCourseConfigPage;  