...
class ZhuangBookPage extends React.Component {
  constructor(props) {
      super(props);
      this.state = {
        dateZhuang:QIndexPage.GetZhuangBook(),
        datahistory:[],
        keyindex: 0
      };
  ...
  static navigationOptions = ({navigation})=>{
    ...
    return{
      ...
      title: RouteConfig["ZhuangBookPage"].titlename,
    }
  };
  ...
  render(){
    ...
    var constZhuang= new Array()
    for(var key in this.state.dateZhuang){
      constZhuang[key]=this.state.dateZhuang[key]
    }
      if(this.state.keyindex<0)
      {
        this.setState({keyindex:constZhuang.length-1})
        return(<View/>)
      }
      if(this.state.keyindex>=constZhuang.length)
      {
      ...
      itemArr = constZhuang.map(function(_, i,arr) { 
        return i;
      })
      .map((_i, index) => {
          ...
          return (
            ...
            >
              ...
              >
                  ...
                  <Text style={{fontSize:FontStyleConfig.getFontApplySize()+22}}>{ constZhuang[index].name}</Text>
        ...
        return (
          ...
          >
            ...
            <Button
                ...
                >
                   <Text style={{fontSize:FontStyleConfig.getFontApplySize()+18}}>{constZhuang[index].name  }{index==this.state.keyindex?IconConfig.IconStar:""}</Text>
                </Button>
          </List.Item>
        );
      });
      ...
      var curZhuang = new Array()
      curZhuang.push(constZhuang[this.state.keyindex].name )
      if(undefined != constZhuang[this.state.keyindex].content)
      {
        curZhuang=curZhuang.concat(constZhuang[this.state.keyindex].content)
      }
      curZhuang.push("")
      curZhuang.push("")
      curZhuang.push("")
