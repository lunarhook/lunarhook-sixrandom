...
class ShuoWenBookPage extends React.Component {
  constructor(props) {
      super(props);
      this.state = {
        dateShuoWen:QIndexPage.GetShuoWenBook(),
        datahistory:[],
        keyindex: 0
      };
  ...
  static navigationOptions = ({navigation})=>{
    ...
    return{
      ...
      title: RouteConfig["ShuoWenBookPage"].titlename,
    }
  };
  ...
  render(){
    ...
    var constShuoWen= new Array()
    for(var key in this.state.dateShuoWen){
      constShuoWen[key]=this.state.dateShuoWen[key]
    }
      if(this.state.keyindex<0)
      {
        this.setState({keyindex:constShuoWen.length-1})
        return(<View/>)
      }
      if(this.state.keyindex>=constShuoWen.length)
      {
      ...
      itemArr = constShuoWen.map(function(_, i,arr) { 
        return i;
      })
      .map((_i, index) => {
          ...
          return (
            ...
            >
              ...
              >
                  ...
                  <Text style={{fontSize:FontStyleConfig.getFontApplySize()+22}}>{ constShuoWen[index].name}</Text>
        ...
        return (
          ...
          >
            ...
            <Button
                ...
                >
                   <Text style={{fontSize:FontStyleConfig.getFontApplySize()+18}}>{constShuoWen[index].name  }{index==this.state.keyindex?IconConfig.IconStar:""}</Text>
                </Button>
          </List.Item>
        );
      });
      ...
      var curShuoWen = new Array()
      curShuoWen.push(constShuoWen[this.state.keyindex].name )
      if(undefined != constShuoWen[this.state.keyindex].content)
      {
        curShuoWen=curShuoWen.concat(constShuoWen[this.state.keyindex].content)
      }
      curShuoWen.push("")
      curShuoWen.push("")
      curShuoWen.push("")
...
module.exports=ShuoWenBookPage;  